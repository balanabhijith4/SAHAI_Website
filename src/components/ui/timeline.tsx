    ...
    >
      <div ref={ref} className="relative max-w-7xl mx-auto pb-10">
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start w-20 md:w-32 shrink-0">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-canvas flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-border border border-border p-2" />
              </div>
              <h3 className="hidden md:block md:pl-16 text-xl md:text-3xl font-bold text-ink-soft/40">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-0 w-full">
              <h3 className="md:hidden block text-xl mb-4 text-left font-bold text-ink-soft/40">
                {item.title}
              </h3>
              {item.content}
            </div>
